...
const int Odorless::Engine::UI::Fonts::FontManager::AddFont(const char *path)
{
	char szTGAPath[255];
	...
	else
	{
		...
		const int cintFontSize = 256;
		const int cintFontCharSize = cintFontSize >> 4;

		//	Generate the uv coordinates for each character.
		for (int y = 0; y < cintFontSize; y+=cintFontCharSize)
		{
			for (int x = 0; x < cintFontSize; x+=cintFontCharSize)
			{
				...
				fontAddFont._vChars.push_back(characterChar);
			}
		}
		CalculateProperUVs(fontAddFont);
...
void Odorless::Engine::UI::Fonts::FontManager::CalculateProperUVs(_FONT &font)
{
	/*
...
void Odorless::Engine::UI::Fonts::FontManager::Write(const char* string) const
{
	...
	unsigned int uintLength = (unsigned int)strlen(string);
